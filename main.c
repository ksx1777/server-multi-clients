...
//#define TIMEOUT_SEC 0x5

#define RECV_LENGTH 1024 * 64
...
int socket_bind(char *ip, short port, int queue_length){
	...
	if(setsockopt(s, SOL_SOCKET, SO_REUSEADDR, &(int){1}, sizeof(int)) != FALSE) err(EXIT_FAILURE, "\0");
	else if(bind(s, (struct sockaddr*)&server_info, sizeof(server_info)) != FALSE) err(EXIT_FAILURE, "\0");
...
int init_new_conn(int s, int conn_num){
	...
	if(socket_fd == -1) err(EXIT_FAILURE, "\0");
	
/*	struct timeval tv;
	tv.tv_sec = TIMEOUT_SEC;
	tv.tv_usec = 0;
	if(setsockopt(socket_fd, SOL_SOCKET, SO_RCVTIMEO, (const char*)&tv, sizeof tv) != FALSE) err(EXIT_FAILURE, "\0");
*/
...
int recv_packet(int conn_num, char *store, int length){
	return recv(connected[conn_num], store, length, MSG_DONTWAIT);
}
...
void *connections_handler(void *arg){
	...
	int c = 0;
	int bytes = 0;
	char * packet_send = (char*)malloc(RECV_LENGTH * sizeof(char));
	char * packet_recv = (char*)malloc(RECV_LENGTH * sizeof(char));
	while(TRUE){
		...
		while(TRUE){
			...
			usleep(10 * 1000);
			mset(packet_recv, 0x0, sizeof(packet_recv));
			mset(packet_send, 0x0, sizeof(packet_send));

			/* enviar dados para o cliente */
			if(send_packet(c, "Teste\n", 6) == -1) break;

			/* recebimento de dados do cliente */
			bytes = recv_packet(c, packet_recv, RECV_LENGTH);
			if(bytes > 0){
				printf("[*] %i bytes recebidos de endereço: %s:%i (%i)\n", bytes, inet_ntoa(client_list[c].sin_addr), client_list[c].sin_port, c+1);
				/* etc... */
			}
		}
		/* final do loop while */
...
int main(int argc, char** argv){
	...
	if(pthread_mutex_init(&queue_mutex, NULL)) err(EXIT_FAILURE, "%s", "Não foi possível iniciar a fila de threads.\n");
	...
	pthread_t *threads = calloc(socket_conf->max_conn, sizeof(pthread_t));
  	...
  	for(int i = 0; i < socket_conf->max_conn-1; i++) if(pthread_create(&threads[i], NULL, connections_handler, NULL)) err(EXIT_FAILURE, "%s", "Erro ao criar thread");
